# -*- coding: windows-1252 -*-
from zlipy.huffman import Tree, reverse_bits
...
def match_expand(out, length, distance):
    ...
    if DEBUG: print(f'expanding match of ({length}, -{distance})')
    if length >= distance:
        s = length * out[-distance:] # ripete la sequenza (finale)
        out.extend(s[:length]) # la limita a length
    else:
        if DEBUG: print('copied', out[-distance:-distance+length])
...
def read_dynamic_trees(b):
    ...
    for i in range(dynCodesLen):
        ...
        clc = b.read(3)
        l[cli] = clc
        if DEBUG and clc: print (f"code {cli} {clc}")
    ...
    while i < (litLenSize + distSize):
        ...
        if 0 <= S <= 15: # length itself
            ...
            n = 3 + b.read(2)
            l = lengths[-1]
            if DEBUG: print(f'repeat {n}')
        elif S == 17: # repeat zeros (few)
            n = 3 + b.read(3)
            l = 0
            if DEBUG: print(f'zeros {n}')
    ...
    if DEBUG: print (f"Dynamic Huffman tables end @{b.tell()}")
    return Tree(lengths[:litLenSize]), Tree(lengths[litLenSize:])

def decompress(s, wbits=-15, return_lz=False):
    ...
    while 1:
        ...
        while 1:
            ...
            if sym == 256:
                ...
                if DEBUG: print('extra bits required for match length:', ebits)
                ebits = b.read(ebits)
                length = deflate_base_lenghts[sym-257] + ebits
                if DEBUG: print('match length is', length)
                ...
                if 0 <= d <= 29:
                    ...
                    if DEBUG: print ('extra bits required for match distance:', ebits)
                    distance = deflate_base_distances[d] + b.read(ebits)
                    if DEBUG: print('match distance is', distance)
                ...
                if DEBUG: print(f'found literal "{sym}"')
